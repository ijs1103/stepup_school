...
const ChallengeListItem = ({ isDone, imageUrl }: Props) => {
    const pressHandler = () => { };
    return (
        <Pressable onPress={pressHandler} style={styles.container}>
            <View style={styles.hStack}>
                ...
                <View style={styles.contentsContainer}>
                    <View style={{ gap: 6 }}>
                        <Text style={styles.title} numberOfLines={1}>{'(설정이름) 하루 만 보 챌린지'}</Text>
                        <Text style={styles.date}>{'5시간 뒤 종료'}</Text>
                    </View>
                    <View style={{ gap: 6 }}>
                        <Text style={styles.total}><Text style={styles.achievementRate}>{'3,444'}</Text>{' / 10,000 걸음'}</Text>
