...
import { BodyInfo } from '@/entities/user/model/stores/useUserSignupStore';
...
const BodyInfoScreen = () => {
    ...
    const [bodyInfo, setBodyInfo] = useState<OptionalBodyInfo>({ gender: undefined, birthDate: undefined, height: undefined, weight: undefined });
    ...
    const pressHandler = useCallback(async (info: keyof BodyInfo) => {
        setCurrentInfo(info);
        openBottomSheet();
    }, [openBottomSheet]);
    ...
    return (
        <View style={styles.container}>
            <View>
                <MagicStick />
                <Spacer size={20} />
                <Text style={styles.title}>{'칼로리 계산을 위한\n개인 정보를 입력해 주세요'}</Text>
                <Spacer size={3} />
                <Text style={styles.subTitle}>{'신체 정보는 데이터 분석의 중요한 근거가 되므로\n정확하게 기입해 주세요'}</Text>
                <Spacer size={38} />
                <View style={styles.infoInputButtonContainer}>
                    <InfoInputButton title={'성별'} info={bodyInfo.gender} pressHandler={() => pressHandler('gender')} />
                    <InfoInputButton title={'생년월일'} info={formatDate(bodyInfo.birthDate)} pressHandler={() => pressHandler('birthDate')} />
                    <InfoInputButton title={'신장'} info={bodyInfo.height && `${bodyInfo.height} cm`} pressHandler={() => pressHandler('height')} />
                    <InfoInputButton title={'체중'} info={bodyInfo.weight && `${bodyInfo.weight} kg`} pressHandler={() => pressHandler('weight')} />
                </View>
            </View>
            ...
            <CustomSheet isOpen={isBottomSheetOpen} onClose={handleBottomSheetClose}>
                ...
                {currentInfo === 'weight' && <WeightInput submitHandler={bottomSheetSubmitHandler} />}
            </CustomSheet>
        </View>
    );
};
