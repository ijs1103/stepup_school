...
import {useUserSignupStore, SchoolInfo} from '@/features/user/model/stores';
...
import {
  ClassInfoResponse,
  useClassInfo,
} from '@/features/schoolInfo/model/useClassInfo';
...
const SchoolInfoScreen = () => {
  ...
  const [schoolInfo, setSchoolInfo] = useState<OptionalSchoolInfo>({
    name: undefined,
    grade: undefined,
    class: undefined,
  });
  ...
  const numberOfClasses = useMemo(() => {
    return classInfo && schoolInfo.grade
      ? classInfo[schoolInfo.grade as keyof ClassInfoResponse] ?? 15
      : 15;
    ...
    <View style={styles.container}>
      <View>
        ...
        <View style={styles.infoInputButtonContainer}>
          <InfoInputButton
            ...
            pressHandler={() => pressHandler('name')}
          />
          <InfoInputButton
            title={'학년'}
            info={schoolInfo.grade}
            pressHandler={() => pressHandler('grade')}
          />
          <InfoInputButton
            title={'학급'}
            info={schoolInfo.class}
            pressHandler={() => pressHandler('class')}
          />
        </View>
      </View>
      ...
      <CustomSheet isOpen={isBottomSheetOpen} onClose={handleBottomSheetClose}>
        {currentInfo === 'name' && (
          <SchoolNameInput
            data={schoolNameList}
            submitHandler={bottomSheetSubmitHandler}
          />
        )}
        {currentInfo === 'grade' && (
          <SchoolGradeInput
